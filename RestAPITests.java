...
public class RestAPITests {
	...
	public void read_from_Testdata_file_and_convert_to_json_Object(String arg1) {
		...
		if (createUserPayload.isEmpty())
			RunCukesTest.logger.log(LogStatus.FAIL, HTMLReportUtil.failStringRedColor("Failed to load test data from excel"));
		else
			RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor("Test data sucessfully loaded from excel"));
	}
	...
	public void i_set_the_content_type_and_body() {
		try {
			...
			RunCukesTest.logger.log(LogStatus.FAIL, HTMLReportUtil.failStringRedColor("URI, Authorization token and Content Type not set."));
	...
	public void i_verify_the_json_response() {
		...
		try {
			...
			if (id != 0) {
				RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor("Response body have an Id " + id));
			} else {
				RunCukesTest.logger.log(LogStatus.FAIL, HTMLReportUtil.failStringRedColor("Response body does not have an Id"));
			}
		} catch (Throwable e) {
			GlobalUtil.errorMsg = e.getMessage();
	...
	public void i_verify_the_response_body_have_code_something(int expectedCode) {
		...
		try {
			...
			RunCukesTest.logger.log(LogStatus.PASS, HTMLReportUtil.passStringGreenColor("Code has been matched. " + code));
		} catch (Throwable e) {
			GlobalUtil.errorMsg = e.getMessage();
			RunCukesTest.logger.log(LogStatus.FAIL, HTMLReportUtil.failStringRedColor("Code did not match. Expected [" + expectedCode + "] but found [" + code + "]"));
			Assert.fail(e.getMessage());
